...
var rules:RefCounted;var state:Dictionary={};var rng:RandomNumberGenerator=RandomNumberGenerator.new();var match_end_emitted:bool=false
func initialize(engine:RefCounted,players:Array,seed:int,config:Dictionary={})->void:
	rules=engine;rng.seed=seed
	if engine is CaxetaRules:state=rules.create_initial_state(players,rng,int(config.get("lives",7)))
