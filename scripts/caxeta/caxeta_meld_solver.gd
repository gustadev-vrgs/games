...
func can_partition_into_melds(cards: Array, wild: Dictionary) -> Dictionary:
	_memo.clear(); var result := _solve(cards.duplicate(),wild)
	return {"valid":result != null,"melds":[] if result == null else result}
func _solve(cards: Array, wild: Dictionary) -> Variant:
	...
	var key := PackedInt32Array(cards.map(func(card: Dictionary) -> int: return card.uid))
	...
	for meld in _candidate_melds(cards,wild):
		...
		var remaining := cards.filter(func(card: Dictionary) -> bool:
			for item in meld:
				if item.uid == card.uid: return false
			return true)
		var rest := _solve(remaining,wild)
		if rest != null:
			var answer: Array = [meld]; answer.append_array(rest); _memo[cache_key] = answer; return answer
	...
	for a in range(cards.size()):
		for b in range(a+1,cards.size()):
			for c in range(b+1,cards.size()):
				var triple := [cards[a],cards[b],cards[c]]
...
func is_set(cards: Array, wild: Dictionary) -> bool:
	...
	var rank := ""; var suits := {}; var wilds := 0
	for card in cards:
...
func is_run(cards: Array, wild: Dictionary) -> bool:
	if cards.size() < 3: return false
	var naturals: Array = []; var wilds := 0; var suit := ""
	for card in cards:
		if _is_wild(card,wild): wilds += 1; continue
