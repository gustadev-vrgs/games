...
func compare_truco_cards(a: Dictionary, b: Dictionary, manilha: String) -> int:
	...
	if am and bm:
		return sign(DeckBuilder.SUITS.find(a.suit) - DeckBuilder.SUITS.find(b.suit))
	if am:
		return 1
	if bm:
		return -1
...
func _play_card(state: Dictionary, actor_id: int, uid: int) -> void:
	...
	if state.played.size() < state.players.size():
		...
		return
	var best: Dictionary = state.played[0]
	var tied: bool = false
	for play_value: Variant in state.played.slice(1):
		...
		if comparison > 0:
			...
			tied = true
	var winner_team: int = DRAW if tied else int(best.team)
	state.trick_results.append(winner_team)
	state.pending_resolution = hand_result(state.trick_results)
	state.reveal_result = {"trick_number":state.trick_number,"winner_team":winner_team,"tied":tied,"winning_peer":-1 if tied else int(best.peer_id),"hand_winner":state.pending_resolution,"points":state.accepted_value}
