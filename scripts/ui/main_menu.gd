...
func _ready()->void:
	super();%Host.pressed.connect(_host);%Join.pressed.connect(_join);%Help.pressed.connect(func()->void:SceneRouter.request_transition("help"));%Quit.pressed.connect(func()->void:get_tree().quit())
func _save()->bool:
