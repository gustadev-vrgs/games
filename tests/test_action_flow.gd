...
func run(t: TestHelpers) -> void:
	...
	var uno_scene_source: String = FileAccess.get_file_as_string("res://scenes/uno/uno_game.tscn")
	t.check(uno_scene_source.contains("name=\"DeclareUno\" type=\"Button\"") and not uno_scene_source.contains("name=\"DeclareUno\" type=\"CheckButton\""), "UNO usa botão simples sem indicador deslizante")
	var uno_pile_source: String = FileAccess.get_file_as_string("res://scripts/uno/uno_draw_pile.gd")
	t.check(not uno_scene_source.contains("Comprar uma carta"), "monte visual substitui o botão textual grande do Uno")
	t.check(uno_scene_source.contains("uno_draw_pile.gd"), "cena do Uno usa o componente visual de monte")
	t.check(uno_pile_source.contains("CARD_OFFSETS") and uno_pile_source.contains("set_available"), "monte desenha cartas sobrepostas e preserva estado desabilitado")
	...
	var results_source: String = FileAccess.get_file_as_string("res://scripts/ui/results_screen.gd")
	t.check(results_source.contains("%ChangeGame.visible = is_host") and results_source.contains("Aguardando o host decidir"), "resultado separa ações do host e espera do cliente")
	t.check(network_source.contains("func _start_next_match") and network_source.contains("_cancel_match_resources()"), "próxima partida limpa recursos sem encerrar a sessão")
