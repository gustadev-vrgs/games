...
func run(t:TestHelpers)->void:
	...
	var open_hand: Dictionary = rules.create_initial_state([1,2], rng)
	t.check(rules.validate_action(open_hand, 1, {"type":"RUN"}).accepted, "jogador da vez pode correr sem pedido")
	rules.apply_action(open_hand, 1, {"type":"RUN"}, rng)
	t.equal(open_hand.scores[1], 1, "correr fora de pedido concede o valor atual ao adversário")
