...
func run(t:TestHelpers)->void:
	...
	var rng: RandomNumberGenerator=RandomNumberGenerator.new();rng.seed=9;var state: Dictionary=CaxetaRules.new().create_initial_state([1,2],rng,7);t.equal(CaxetaRules.new().validate_invariants(state),"OK","conservação Caxeta")
